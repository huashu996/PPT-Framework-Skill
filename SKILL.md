...
## PowerPoint 启动与公式输入的强制前置闸门

以下两项属于界面操作的强制前置条件；任意一项未通过时，禁止继续制作：
...
3. **每条公式正式输入前必须先通过英文输入状态检查。** 新建或打开 MathType 编辑器后，先在空白公式中逐键输入拉丁字母 `x`，并直接检查编辑区。只有同时满足“仅显示一个英文 `x`”“没有中文候选栏”“没有拼音预编辑串、组合下划线或待选状态”时才算通过。出现英文 `x` 但仍显示中文候选栏，同样判定为中文输入状态，禁止开始公式。
4. 不得根据 PowerPoint/MathType 界面语言、上一条公式的输入状态、任务栏图标或按键已发送来推断输入法正确。必须以当前 MathType 编辑区中本条公式的可见 `x` 测试结果为准。测试失败时取消候选并清空，只允许切换到英文后重测一次；无法明确验证通过时停止并报告输入法阻断。通过前产生的任何公式字符均视为无效，必须全部清空。
...
- MathType 输入前必须把输入法冻结为英文状态。在空公式中逐键输入一个拉丁 `x`，并确认只显示英文 `x` 且没有中文候选栏、拼音预编辑串、组合下划线或待选状态；英文 `x` 与中文候选栏同时出现仍属于测试失败。只允许取消候选并清空一次、切换英文状态后重测一次。测试成功后冻结输入法和逐键输入方式，不得在同一批公式中来回切换中文输入法、剪贴板粘贴、TeX 粘贴和不同键盘路线。
...
2. 使用权威 PPTX 的绝对路径直接启动 PowerPoint；若该路径已打开，则激活并复用标题匹配的现有窗口。禁止通过“打开”对话框、截图定位、输入路径或目录浏览打开文件。随后最大化 PowerPoint 并确认标题栏与目标文件一致；窗口最小化、标题不匹配或焦点在其他应用时，先恢复窗口，不做任何试探性点击。
3. 从当前 PowerPoint 中可见且启用的 `MathType` 插件进入。优先按可访问名称精确调用“插入公式”组内的新公式按钮；禁止复用旧坐标、旧索引，禁止点击 `MathType 帮助`、`网络上的 MathType`、浏览器或网页。
4. 每创建或编辑一条公式，都先在空白 MathType 中逐键输入拉丁字母 `x`。只有画面显示单独英文 `x`，并且没有中文字符、中文候选栏、拼音预编辑串、组合下划线或待选状态时，才清空 `x` 并开始正式输入。只看到英文 `x` 但候选栏仍存在时，测试不通过。
5. 若 `x` 触发中文候选或任何预编辑状态，只执行一次：取消候选、清空、切换英文、重新输入 `x` 并再次目视确认。复测仍失败或无法确认时立即停止该公式并报告输入法阻断，不得继续盲输；通过前输入的任何公式内容必须全部丢弃。
